...
async function p02ClientHints(){
     ...
     try {
       ...
       // Send raw uach data as comma-separated key:value pairs instead of JSON hash
       const uachPairs = Object.entries(h).map(([k,v])=>k+":"+v);
       feed("uach", uachPairs.join(","));
     } catch(e){}
   }
...
async function p05Canvas(){
     try{
       ...
       // Send raw dataURL (or a reasonable substring) instead of hash
       feed("canvas", data);
     } catch(e){}
   }
...
async function p06WebGL(){
     ...
     // Send raw extensions list instead of hash
     feed("glext", exts.join(","));
   }
  ...
  async function p08Audio(){
    ...
    try {
      ...
      feed("audio", sum.toString()); // send raw sum value instead of hash
    } catch(e){}
  }
  ...
  async function p09Fonts(){
    ...
    // Send raw fonts list instead of hash
    feed("fonts", found.join(","));
  }
...
const line = [];
     ...
     // Send raw codecs list with commas instead of hash
     feed("codecs", line.join(","));
  }
  ...
  async function p11Voices(){
    ...
    // Send raw voices list instead of hash
    feed("voices", names.join(","));
  }
  ...
  async function p17API(){
    ...
    // Send raw features as comma-separated key:value pairs instead of hash
    const featPairs = Object.entries(feats).map(([k,v])=>k+":"+v);
    feed("apis", featPairs.join(","));
  }
...
const wa = window.WebAssembly;
     ...
     // Send raw wasm features plus hardwareConcurrency as comma-separated key:value pairs
     const featPairs = Object.entries(feats).map(([k,v])=>k+":"+v);
     featPairs.push("hwConcurrency:"+navigator.hardwareConcurrency);
     feed("wasm", featPairs.join(","));
  }
...
document.body.removeChild(host);
    // Send raw rects data as comma-separated values instead of hash
    const rectsData = [...vals.map(n => (+n).toFixed(4)), ...bbox.split(", ").map(n => (+n).toFixed(3)), tlen];
    feed("rects", rectsData.join(","));
  }
  ...
  async function p33TextMetrics(){
   ...
   // Send raw text metrics instead of hash
   feed("tmetrics", vals.join(","));
  }
